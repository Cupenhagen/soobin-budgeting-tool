...
const ACTION_REGEX = /\[TIARA_ACTION:\s*add_transaction\s*\|([^\]]+)\]/gi
...
export async function executeAction(action: ParsedAction): Promise<void> {
  if (action.type !== 'add_transaction') return
  ...
  // Find first non-archived account
  const allAccounts = await db.accounts.toArray()
  const account = allAccounts.find((a) => !a.isArchived) ?? allAccounts[0]
